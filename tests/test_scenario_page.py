...
# --- Assistant "apply" recovery: caption, layer notice, revert (issue #58) ---


def _apply_both_artifacts(fake_session_state, *, page_id: str = "threat_group") -> None:
    """Simulate the Assistant page's apply flow against a persisted result."""
    stash_pre_apply(page_id)
    apply_write_back(
        page_id=page_id,
        artifact="scenario",
        revised_text="# Refined APT29 Scenario",
        session_state=fake_session_state,
    )
    apply_write_back(
        page_id=page_id,
        artifact="defense",
        revised_text="## Refined walkthrough",
        session_state=fake_session_state,
    )
    mark_applied(page_id)


def test_no_revert_button_or_caption_before_any_apply(
    stub_streamlit, fake_session_state, monkeypatch: pytest.MonkeyPatch
) -> None:
    _generate_threat_group_result(stub_streamlit, fake_session_state, monkeypatch)
    ...
    assert not any("Refined via the AttackGen Assistant" in c for c in stub_streamlit["captions"])


def test_revert_button_and_caption_appear_after_an_apply(
    stub_streamlit, fake_session_state, monkeypatch: pytest.MonkeyPatch
) -> None:
    _generate_threat_group_result(stub_streamlit, fake_session_state, monkeypatch)
    ...
    assert any(
        b.get("key", "").startswith("threat_group_revert_")
        and b.get("label") == "Revert to original"
        for b in stub_streamlit["buttons"]
    )
    assert any("Refined via the AttackGen Assistant" in c for c in stub_streamlit["captions"])
    assert fake_session_state["threat_group_scenario_text"] == "# Refined APT29 Scenario"


def test_layer_caption_notes_it_reflects_the_original_scenario_after_apply(
    stub_streamlit, fake_session_state, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Constraint: the Navigator layer is never regenerated by an apply."""
    _generate_threat_group_result(stub_streamlit, fake_session_state, monkeypatch)
    _apply_both_artifacts(fake_session_state)
...
def test_revert_restores_the_scenario_and_narrative_together(
    stub_streamlit, fake_session_state, monkeypatch: pytest.MonkeyPatch
) -> None:
    _generate_threat_group_result(stub_streamlit, fake_session_state, monkeypatch)
    original_text = fake_session_state["threat_group_scenario_text"]
    original_defense = fake_session_state["threat_group_scenario_defense"]
    assert original_defense["narrative_md"] is None  # deterministic-only, pre-apply

    _apply_both_artifacts(fake_session_state)
    assert fake_session_state["threat_group_scenario_text"] == "# Refined APT29 Scenario"
    assert fake_session_state["threat_group_scenario_defense"]["narrative_md"] == (
        "## Refined walkthrough"
    )

    # A plain rerun renders the (now-visible) Revert button as "previous" —
    # nothing was generated on *this* run, so the just-applied result renders
    # through `_render_previous`, not the "current" path a fresh generation uses.
    stub_streamlit["button_returns"] = False
    run_scenario_page(
        page_id="threat_group",
        build_messages=lambda _s: None,
    ...
    defense = fake_session_state["threat_group_scenario_defense"]
    assert defense["narrative_md"] is None
    assert defense["deterministic_md"] == original_defense["deterministic_md"]
    assert defense["download_md"] == original_defense["download_md"]
    # The Assistant handoff moved back with it.
    assert fake_session_state["last_scenario_text"] == original_text
    assert fake_session_state["last_defense_narrative"] is None
    # No longer flagged as applied, so the notice and the button are gone.
    assert not has_been_applied("threat_group")
    assert applied_caption("threat_group") is None
    assert not any(
        b.get("key", "").startswith("threat_group_revert_") for b in stub_streamlit["buttons"]
    )
    assert downloads[0]["data"] == original_text


def test_revert_does_not_appear_when_nothing_was_ever_applied(
    stub_streamlit, fake_session_state, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Clicking a revert that was never rendered (e.g. a stray flag) must be a no-op."""
    _generate_threat_group_result(stub_streamlit, fake_session_state, monkeypatch)
    fake_session_state["threat_group_revert_requested"] = True
...
def test_clear_result_also_clears_apply_recovery_state(
    stub_streamlit, fake_session_state, monkeypatch: pytest.MonkeyPatch
) -> None:
    _generate_threat_group_result(stub_streamlit, fake_session_state, monkeypatch)
    _apply_both_artifacts(fake_session_state)
