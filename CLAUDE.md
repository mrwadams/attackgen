...
- **core/**: Unified LLM wrapper. All provider integration lives here — pages never call provider SDKs directly.
  ...
  - **core/attack_data.py**: Headless MITRE loaders + kill-chain resolution, shared by the pages and the MCP server. Lazy `@lru_cache` singletons (`enterprise_data`/`ics_data`/`atlas_data`) with `__file__`-derived absolute paths (so the MCP server works from any cwd and the 53 MB Enterprise bundle loads only on first use). `resolve_kill_chain(matrix, group, *, seed=None)` is the one entry point callers outside `core` use — it dispatches ATLAS case studies to `resolve_case_study_kill_chain` and Enterprise/ICS threat groups to `resolve_threat_group_kill_chain` internally, and raises `ValueError` for an unknown matrix, so no caller branches on the matrix itself. `resolve_threat_group_kill_chain` reproduces the old page-1 logic exactly — dedupe for display, phase normalisation/ordering, and one-technique-per-phase sampling from the *non-deduplicated* set (pass `seed` for a deterministic draw; `None` = the UI's per-run randomness). `resolve_case_study_kill_chain` handles ATLAS (full procedure, no sampling). Both return a JSON-native `KillChain`, whose `technique_ids` property is the bare-ID form of `techniques` — the one place that derivation happens.
  - **core/navigator.py**: Builds an ATT&CK Navigator layer JSON from a scenario's techniques, offered as a second download next to the markdown on pages 1–2. Maps each matrix to its Navigator domain (`enterprise-attack`, `ics-attack`, and the ATLAS Navigator fork's `atlas-atlas`); ATLAS layers omit the `attack` version field. Pages pass a `build_layer` callback to `run_scenario_page`; the layer is captured at generation time and persisted so it can't drift from the scenario on rerun. `normalise_technique_ids(techniques)` turns a mix of `"Name (ID)"` display labels and bare IDs into bare IDs — the shared form the Custom Scenarios page and the MCP server's `techniques`/`technique_ids` arguments both normalise through.
  - **core/detections.py**: Purple-team "Detection & Response" companion. For a scenario's techniques it joins the defensive half of the STIX bundle already shipped — ATT&CK v18+ detection strategies + analytics (with their log sources) and mitigations — into a deterministic Markdown section (no LLM call). `resolve_defense_report(matrix, technique_ids)` is the one entry point pages and the MCP server call — it resolves the right `core.attack_data` bundle for the matrix itself (Enterprise/ICS via `mitreattack-python` helpers; ATLAS degrades to mitigations only, it has no detection model) and delegates to `build_defense_report`, which keeps its `mitre_data`/`atlas_data` parameters as the injection seam the detections and MCP tests use. Pages pass a `build_defense` callback to `run_scenario_page`; the report is captured at generation time and persisted (like the layer) so it can't drift on rerun, and offered as a `..._detection.md` download. Also hosts an optional LLM "purple-team narrative" pass (a per-page `🟣 Purple-team narrative` toggle) that weaves those *supplied* detections/mitigations into a stage-by-stage defender's walkthrough — a second model call tagged `purple_team_narrative` in LangSmith.
  - **core/scenario_page.py**: `run_scenario_page(...)` — the shared lifecycle coordinator behind all three scenario pages. Owns readiness (see `core/readiness.py`), the phased generation run, persistence of the base result before optional enrichment, the result summary/navigation surface, the one result-action area (Open in Assistant, downloads, Regenerate, Clear result), retries and the Assistant handoff. Pages own their widgets, prompt assembly and their own `requirements`. Result state lives under `{page_id}_scenario_*` session keys — never widget keys — so a result survives reruns and navigation for the session.
  - **core/readiness.py**: Readiness as data. `resolve_readiness(requirements=..., setup=...)` merges a page's own blockers with the shared Setup blockers (page-specific first) into a `Readiness`; the coordinator renders the summary and derives Generate's disabled state from it, so validation is visible before the click and no model call can start from an incomplete form.
  - **core/summary.py**: Streamlit-free views over a result and its inputs. `summarise_scenario` extracts an overview, section index and facilitator shortcuts (injects, discussion questions, success criteria, metrics, artefacts, rules of engagement) for the navigation surface above the full Markdown; `describe_inputs`/`summary_line` render the Generate-time snapshot both as the pre-flight confirmation and as the result's "Generated from" metadata; `inputs_changed` powers the stale-result notice and deliberately ignores fields a page re-derives each rerun (page 1's per-phase technique sample).
  - **core/routes.py**: The page registry (`SCENARIO_PAGES`, `ASSISTANT_PAGE`, `page_info`). One place holds each page's script path so cross-page links — the result area's Open in Assistant, the Assistant's Back to this scenario — never hard-code a filename.
  - **core/assistant.py**: The Assistant's state contract, prompts and model seam. `scenario_handoff()` resolves the `last_scenario*` handoff written by the coordinator (text, optional purple-team narrative, and metadata naming the result and its originating page); `render_empty_state()` offers the three ways to create a scenario instead of a dead end; `build_assistant_messages`/`stream_assistant_reply` are the testable prompt and streaming seams. Page 4 is a thin renderer over it.
  - **core/sidebar.py**: The shared Setup sidebar rendered by every page, plus `get_setup_state(...)` — the pure state contract returning the resolved provider/credential/model/matrix/industry/size and every blocker, testable without rendering a widget.
  - **core/state.py**: Non-secret setup persistence via `st.query_params` (provider, model, base URL, matrix, industry, size). API keys, generated scenarios, chat history and artefacts are never mirrored to the URL. A deep link that supplies setup sets a marker so a page with no result can explain that scenarios are session-only.
...
- **pages/**: Streamlit pages for different functionality. Each scenario page renders the shared Setup sidebar, its own widgets, its `requirements` (page-specific readiness blockers), its modifiers *above* the Generate button, and delegates the rest to `run_scenario_page`
  ...
  - **4_💬_AttackGen_Assistant.py**: Chat interface for refining scenarios. A thin renderer over `core/assistant.py`: with no scenario it links to all three generation pages; with one it names the scenario being discussed and links back to the page that produced it.
