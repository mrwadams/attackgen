...
| Model Registry Refresh | - Latest Models: The model list is refreshed against each provider's current docs — **Claude Opus 5** and **Claude Fable 5.1** join the Anthropic list, and Google's fast tiers move to **Gemini 3.7 Flash** and **Gemini 3.5 Flash Lite**, with one prior generation kept for continuity where a provider has one.<br><br>- Removed Retired Entries: Dropped ids the providers have superseded or shut down — Claude Opus 4.7, now three generations behind, and Groq's `llama-3.3-70b-versatile` and `llama-3.1-8b-instant` (both retired 16 Aug 2026) — so the sidebar can't offer a model that only errors once you press **Generate**. Groq's open-weight reasoning tier is now `qwen/qwen3.8-27b`.<br><br>- Temperature Rule Kept Honest: `supports_temperature` is set from each Anthropic model's actual sampling behaviour — every Claude from 4.7 on rejects a custom temperature, and Haiku 4.5 remains the only listed model that accepts one. |
